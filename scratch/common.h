...
// Add new output file variables for flow paths
std::string flow_path_file = "flow_paths.txt";
typedef std::pair<int, std::pair<int, int>> MsgEventKey;
std::map<std::pair<MsgEventKey, int>, std::vector<uint32_t>> flowPathMap;
...
struct RoutingEntry {
    Ipv4Address destination; // Destination network address
    Ipv4Address gateway;     // Gateway address for this route
    uint32_t interface;      // Interface index for this route

    RoutingEntry(Ipv4Address dest, Ipv4Address gate, uint32_t iface)
        : destination(dest), gateway(gate), interface(iface) {}
};

std::map<uint32_t, std::vector<RoutingEntry>> routingTables;
...
/*
//original flowpath generation w/o eventkey
void GenerateFlowPaths(NodeContainer &n, FILE *file) {
    for (uint32_t src = 0; src < n.GetN(); src++) {
        Ptr<Node> srcNode = n.Get(src);
        if (srcNode->GetNodeType() != 0) continue; // Skip non-server nodes

        for (uint32_t dst = 0; dst < n.GetN(); dst++) {
            Ptr<Node> dstNode = n.Get(dst);
            if (dstNode->GetNodeType() != 0 || src == dst) continue; // Skip non-server or same node

            fprintf(file, "Flow from Node %d to Node %d: ", srcNode->GetId(), dstNode->GetId());
            
            auto it = nextHop.find(dstNode);
            if (it != nextHop.end() && it->second.find(srcNode) != it->second.end()) {
                std::vector<Ptr<Node>> path = it->second[srcNode];
                for (Ptr<Node> node : path) {
                    fprintf(file, "%d ", node->GetId());
                }
            }
            fprintf(file, "\n");
        ...
        for (uint32_t dst = 0; dst < n.GetN(); dst++) {
            ...
            if (it != nextHop.end() && it->second.find(srcNode) != it->second.end()) {
                ...
                */
                ...
                // Check for associated events in flowPathMap
                for (const auto& [eventKey, eventPath] : flowPathMap) {
                    MsgEventKey msgKey = eventKey.first;
                    int port = eventKey.second;

                    if (msgKey.second.first == srcNode->GetId() && 
                        msgKey.second.second == dstNode->GetId()) {
                        fprintf(file, "  Event Key: <Tag: %d, Src: %d, Dst: %d, Port: %d>\n",
                                msgKey.first,
                                msgKey.second.first,
                                msgKey.second.second,
                                port);

                        // Print the associated flow path
                        fprintf(file, "    Associated Flow Path: ");
                        for (uint32_t hop : eventPath) {
                            fprintf(file, "%d ", hop);
                        }
                        fprintf(file, "\n");
                    }
...
void SetRoutingEntries() {
  for (auto i = nextHop.begin(); i != nextHop.end(); i++) {
    Ptr<Node> node = i->first;//current dst
    auto &table = i->second; // Map of source nodes and their next hops
    for (auto j = table.begin(); j != table.end(); j++) {
      Ptr<Node> dst = j->first;
      Ipv4Address dstAddr = dst->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal();
      vector<Ptr<Node>> nexts = j->second;//// All possible next hops
      for (int k = 0; k < (int)nexts.size(); k++) {//iterate over all valid next hops
        Ptr<Node> next = nexts[k];
        uint32_t interface = nbr2if[node][next].idx;// Interface for this hop
        if (node->GetNodeType() == 1)//the node is a switch
          DynamicCast<SwitchNode>(node)->AddTableEntry(dstAddr, interface);//packets destined for this address will be sent thru this interface
        else {// if it's a host, consult rdma routing table
...
void InspectNodeRoutingProtocols(NodeContainer nodes) {
    for (uint32_t i = 0; i < nodes.GetN(); ++i) {
        Ptr<Node> node = nodes.Get(i);
        Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();

        if (!ipv4) {
            std::cout << "Node " << i << " does not have an IPv4 stack installed.\n";
            continue;
        }

        Ptr<Ipv4RoutingProtocol> routingProtocol = ipv4->GetRoutingProtocol();
        if (!routingProtocol) {
            std::cout << "Node " << i << " does not have a routing protocol installed.\n";
            continue;
        }
        if (ipv4) {
          Ptr<Ipv4RoutingProtocol> routingProtocol = ipv4->GetRoutingProtocol();
          ...
          if (listRouting) {
              std::cout << "Node " << node->GetId() << " uses Ipv4ListRouting.\n";
              for (uint32_t i = 0; i < listRouting->GetNRoutingProtocols(); ++i) {
                  ...
                  std::cout << "  Sub-protocol " << i << " with priority " << priority << ":\n";
                  if (DynamicCast<Ipv4GlobalRouting>(subProtocol)) {
                      std::cout << "    Ipv4GlobalRouting\n";
                  } else if (DynamicCast<Ipv4StaticRouting>(subProtocol)) {
                      std::cout << "    Ipv4StaticRouting\n";
                  } else {
                      std::cout << "    Unknown routing protocol\n";
                  }
              }
          }
          for (uint32_t i = 0; i < ipv4->GetNInterfaces(); ++i) {
            for (uint32_t j = 0; j < ipv4->GetNAddresses(i); ++j) {
                std::cout << "Node " << node->GetId() << ", Interface " << i
                          << ", Address: " << ipv4->GetAddress(i, j).GetLocal() << "\n";
            }
        }
        }
    }
...
void PopulateRoutingTableVariable() {
    for (NodeList::Iterator nodeIt = NodeList::Begin(); nodeIt != NodeList::End(); ++nodeIt) {
        ...
        if (globalRouting) {
            for (uint32_t i = 0; i < globalRouting->GetNRoutes(); ++i) {
                Ipv4RoutingTableEntry entry = globalRouting->GetRoute(i);
                routingTables[node->GetId()].emplace_back(
                    RoutingEntry(entry.GetDestNetwork(), entry.GetGateway(), entry.GetInterface()));
            }
        }
            ...
            for (uint32_t i = 0; i < listRouting->GetNRoutingProtocols(); ++i) {
                ...
                if (staticRouting) {
                    for (uint32_t j = 0; j < staticRouting->GetNRoutes(); ++j) {
                        Ipv4RoutingTableEntry entry = staticRouting->GetRoute(j);
                        routingTables[node->GetId()].emplace_back(
                            RoutingEntry(entry.GetDestNetwork(), entry.GetGateway(), entry.GetInterface()));
                    }
                }
...
bool ReadConf(string network_configuration) {
  ...
  //if (argc > 2) {
      ...
      //}
    } else if (key.compare("FLOW_PATH_FILE") == 0) {
      std::string v;
      conf >> v;
      flow_path_file = v;
...
bool SetupNetwork(void (*qp_finish)(FILE *, Ptr<RdmaQueuePair>)) {
  ...
  // Open the file for flow paths
  FILE *flowFile = fopen(flow_path_file.c_str(), "w");

  // Generate flow paths
  //PrintFlowPathMap();
  GenerateFlowPaths(n,flowFile);
  fclose(flowFile);
  ...
  PopulateRoutingTableVariable();
  /*
  for (const auto& [nodeId, routes] : routingTables) {
