...
extern std::map<std::pair<MsgEventKey, int>, std::vector<uint32_t>> flowPathMap;
...
// A new map to store events by their MsgEventKey
map<pair<MsgEventKey, int>, MsgEvent> workloadEventHash;
...
void send_flow(int src_id, int dst, int maxPacketCount,
  ...
  //keep a copy of the send_event
  workloadEventHash[send_event_key] = send_event;
  if (flowPathMap.find(send_event_key) == flowPathMap.end()) {
        flowPathMap[send_event_key] = {static_cast<uint32_t>(src_id), static_cast<uint32_t>(dst)};
  }
  
  /*
  std::cout << "Populating flowPathMap: tag = " 
          << send_event_key.first.first << ", Src = " 
          << send_event_key.first.second.first << ", Dst = " 
          << send_event_key.first.second.second 
          << ", Path size = " << flowPathMap[send_event_key].size() << std::endl;
  // Log the event details
    std::cout << "Event stored: src=" << src_id << ", dst=" << dst
              << ", tag=" << tag << ", maxPacketCount=" << maxPacketCount
              << std::endl;
  */
  //original
...
void PrintRoutingTable() {
    ...
    std::cout << "Node " << nodeId << " Routing Table:\n";
